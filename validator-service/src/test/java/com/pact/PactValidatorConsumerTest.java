...
public class PactValidatorConsumerTest {


    @Pact(consumer = "validator-consumer")
    public RequestResponsePact validateUserPact(PactDslWithProvider builder) {
        ...
        return builder
                .given("validate user")
                .uponReceiving("user valll")
                .method(RequestMethod.PATCH.name())
                .path("/api/validate-user/1")
                .body(bodyRequest)
                .willRespondWith()
                .status(200)
    ...
    public void testPact(MockServer mockServer) throws IOException {
        ...
        String entity = new Gson().toJson(validatedUser);
        HttpResponse httpResponse = Request.Patch(mockServer.getUrl() + "/api/validate-user/1")
